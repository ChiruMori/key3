...
import work.cxlm.exception.JsonConvertException;
...
public class JacksonUtil {
    ...
    public <T> T jsonToObject(String content, Class<T> clazz) {
        ...
        try {
            ...
            throw new JsonConvertException("JSON 转化为对象出错，源 JSON：" + content, e);
        }
    }
    ...
    public <T> List<T> jsonToObjectList(String rawJson, Class<T> beanType) {
        ...
        try {
            ...
            throw new JsonConvertException("json 转化为 List 出错，源 JSON：" + rawJson, e);
        }
    }
    ...
    public <T> Map<String, T> jsonToMap(String jsonString, Class<T> clazz) {
        ...
        try {
            Map<String, Map<String, Object>> map = OBJECT_MAPPER.readValue(jsonString, new TypeReference<>() {
            });
            for (Map.Entry<String, Map<String, Object>> entry : map.entrySet()) {
                result.put(entry.getKey(), OBJECT_MAPPER.convertValue(entry.getValue(), clazz));
            }
        } catch (IOException e) {
            throw new JsonConvertException("Json 转化为 Map 出错，源 JSON：" + jsonString, e);
        }
